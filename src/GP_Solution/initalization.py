...
def make_random_tree(
    ...
    left = make_random_tree(max_depth-1, grow, which)
...
def make_weighted_random_tree(
    ...
    op = random.choice(FUNC_SET)
    left = make_weighted_random_tree(max_depth-1, grow, which)
    right = make_weighted_random_tree(max_depth-1, grow, which)
    return NodeGP(op=op, left=left, right=right, which=which)
...
def create_greedy_pop(
    ...
    while len(pop) < pop_size * 2 //3:
        r_tree = make_weighted_random_tree(max_depth, grow=True, which='R')
        s_tree = make_weighted_random_tree(max_depth, grow=True, which='S')
        pop.append(Individual(r_tree, s_tree))
    
    while len(pop) < pop_size:
        use_grow = random.random() < 0.5
        r_tree = make_random_tree(max_depth, use_grow, which='R')
        s_tree = make_random_tree(max_depth, use_grow, which='S')
        pop.append(Individual(r_tree, s_tree))
