...
class PopulationInitializer:
    ...
    def make_random_tree(
        ...
        if max_depth == 1 or (grow and random.random() < 0.4):
            # return NodeGP(terminal=PopulationInitializer._random_terminal(which), which=which)
    ...
    def _make_weighted_random_tree(
        ...
        if max_depth == 1 or (grow and random.random() < 0.4):
            # return NodeGP(terminal=PopulationInitializer.weighted_terminal(which), which=which)
