...
class Simulator:
    ...
    def _handle_veh_free_event(self, payload: Any):
        # if self.enable_logging:
        #     self.log_events.append(f"{self.cur_time:.4f}: VEH_FREE veh {payload}")
        # Parse payload
        if isinstance(payload, tuple):
            vid, action, detail = payload
        else:
            vid = payload
            action = "WAKE_UP" # Mặc định nếu chỉ truyền ID (trường hợp đánh thức)
            detail = None

        # LOG TẠI ĐÂY - ĐÚNG THỜI ĐIỂM
        if self.enable_logging:
            if action == "PICKUP":
                ...
                self.log_events.append(f"{self.cur_time:.4f}: [EVENT] WAKE UP veh {vid} at Depot")
    ...
    def _attempt_assign_to_vehicle(self, req: Request, veh: Vehicle, combined_score_on_veh: float) -> Tuple[bool, List[Request]]:
        ...
        return False, []
        # Nếu tìm được nạn nhân phù hợp -> Thực hiện tráo đổi
        
        # target_removed_req = None
        # # --- [LOGIC MỚI: CHỈ ĐÁ 1 ĐƠN] ---
        # for q, q_score in removable:
        #     # Kiểm tra: Nếu đá đơn 'q' ra thì có đủ chỗ cho 'req' không?
        #     # Công thức: (Hàng đang có) - (Hàng bị đá) + (Hàng mới) <= Capacity
        #     if sum_top_demand - q.demand + req.demand <= capacity + 1e-6:
        #         target_removed_req = q
        #         break # Tìm thấy 1 người thế mạng hợp lý nhất -> Dừng ngay lập tức
            
        # if target_removed_req:
        #     try:
        #         veh.req_queue.remove(target_removed_req)
        #     except ValueError:
        #         pass
            
        #     # Reset trạng thái đơn bị đá
        #     target_removed_req.is_picked_up = False
        #     target_removed_req.pickup_time = None
            
        #     # Thêm đơn mới vào
        #     veh.req_queue.append(req)
            
        #     if self.enable_logging:
        #         self.log_events.append(f"{self.cur_time:.4f}: REPLACE 1-ON-1 on veh {veh.id}: removed {target_removed_req.id} -> added req {req.id}")
            
        #     if veh.busy_until <= self.cur_time + 1e-6:
        #         self._dispatch_vehicle(veh)
            
        #     # Trả về đơn bị đá dưới dạng list (để tương thích với logic bên ngoài)
        #     return True, [target_removed_req]
    ...
    def _try_assign_request(self, req: Request, vehicles: Iterable[Vehicle], exclude_vehicle_ids: Optional[Set[int]] = None) -> bool:
        ...
        if hasattr(req, 'cached_candidates') and req.cached_candidates is not None:
             # Nếu request này đã từng được tính toán (ví dụ: bị đá ra từ xe khác), dùng lại list cũ
             cand_list = req.cached_candidates
        else:
             # Nếu là request mới tinh, tính toán và lưu lại vào req
             cand_list = self._compute_candidate_list(req, self.problem.vehicles, exclude_vehicle_ids)
        ...
        # Xử lý các request bị đá ra (removed requests)
        while to_reassign_queue:
            ...
            if hasattr(removed_req, 'cached_candidates'):
                ...
                # Fallback phòng trường hợp lỗi (không nên xảy ra nếu logic trên đúng)
                removed_cands = self._compute_candidate_list(removed_req, self.problem.vehicles)            
    ...
    def _dispatch_vehicle(self, veh: Vehicle) -> None:
        ...
        # (Đoạn này giữ nguyên để đảm bảo không vi phạm ràng buộc đơn đang chở)
        if veh.picked_up_orders:
            time_to_depot = veh.moving_time(veh.current_location, (0, 0))
            arrival_at_depot = ready_time + time_to_depot
            for picked in veh.picked_up_orders:
        ...
        for req in veh.req_queue:
            ...
            # Thời điểm muộn nhất phải xuất phát để vừa kịp lúc mở cửa (Just-in-Time)
            # e_i - travel_time
            jit_departure_time = req.time_window[0] - travel_time
            
            # Thời điểm đến nơi nếu đi ngay bây giờ
            ...
            # 1. Quá giờ đóng cửa (l_i)
            if arrival_if_go_now > req.time_window[1] + 1e-6: continue

            # 2. Không kịp về Depot trước khi Depot đóng cửa
            arrival_depot_after = service_start + veh.moving_time(req.location, (0, 0))
            if arrival_depot_after > depot_close + 1e-6: continue

            # 3. Vi phạm l_w (thời gian chờ tối đa trên xe)
            # Check cho các đơn ĐANG trên xe
            if any((arrival_depot_after - p.pickup_time > p.l_w + 1e-6) for p in veh.picked_up_orders):
                continue
            # Check cho đơn MỚI chuẩn bị đón
            if (arrival_depot_after - service_start > req.l_w + 1e-6):
                continue
            ...
            # jit_departure_time: thời điểm lý tưởng để xuất phát
            candidates.append({
                'score': score,
                'req': req,
                'travel_time': travel_time,
        ...
        # 3. Ra quyết định
        if candidates:
            # Sắp xếp chọn đơn tốt nhất theo Score (nhỏ nhất là tốt nhất nếu minimization)
            # Giả sử hàm mục tiêu là minimize score (hoặc sort reverse=True nếu maximize)
            # Ở đây sort tăng dần (nhỏ = ưu tiên), tie-break bằng travel_time
            ...
            if veh.current_location == (0.0, 0.0):
                # Nếu thời điểm hiện tại VẪN SỚM HƠN thời điểm cần xuất phát
                if ready_time < best['jit_departure_time'] - 1e-6:
                    ...
                    heapq.heappush(self.event_queue, (wake_up_time, "VEH_FREE", veh.id))
                    return
            
            # --- THỰC HIỆN ĐI ---
            # Nếu đã đến giờ (hoặc muộn hơn), hoặc xe không ở depot -> Đi luôn
            if self.enable_logging:
                # self.log_events.append(f"{ready_time:.4f}: DISPATCH veh {veh.id} from {veh.current_location}")
        ...
        # 4. Nếu không có đơn nào (hoặc không thỏa mãn constraints) và xe đang ở ngoài đường -> Về Depot
        if not candidates and veh.current_location != (0, 0):
            self._process_final_return(veh, ready_time)
